...
async def download_video(client, CallbackQuery):
    ...
    if current_time - user_last_CallbackQuery_time.get(user_id, 0) < SPAM_WINDOW_SECONDS:
        return await CallbackQuery.answer("⏳ ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ 30 sᴇᴄᴏɴᴅs!", show_alert=True)
    ...
    try:
        ...
        # --- SUCCESS MESSAGE WITH DIRECT LINK & BUTTON ---
        await pablo.edit_caption(
            caption=f"**[✅ ᴠɪᴅᴇᴏ sᴇɴᴛ ɪɴ ʏᴏᴜʀ ᴘᴍ!](https://t.me/{app.username})**\n\n**👤 ᴜsᴇʀ:** {mention}",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("📥 ᴄʜᴇᴄᴋ ɪɴ ᴘᴍ", url=f"https://t.me/{app.username}")
            ]])
        )
        await asyncio.sleep(15) # 15 sec wait taaki user button daba sake
        await pablo.delete()
    except Exception:
...
async def download_audio(client, CallbackQuery):
    ...
    try:
        ...
        # --- SUCCESS MESSAGE WITH DIRECT LINK & BUTTON ---
        await pablo.edit_caption(
            caption=f"**[✅ ᴀᴜᴅɪᴏ sᴇɴᴛ ɪɴ ʏᴏᴜʀ ᴘᴍ!](https://t.me/{app.username})**\n\n**👤 ᴜsᴇʀ:** {mention}",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("📥 ᴄʜᴇᴄᴋ ɪɴ ᴘᴍ", url=f"https://t.me/{app.username}")
            ]])
        )
